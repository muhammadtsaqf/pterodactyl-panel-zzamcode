...
const SectionHeader = styled.div`
    ${tw`px-6 pt-5 pb-2 text-xs font-bold tracking-wider text-neutral-500 uppercase`};
`;

const NavItem = styled.div`
    & > a, & > button {
        ${tw`flex items-center w-full px-6 py-3 text-sm font-medium text-neutral-400 transition-all duration-200 relative`};
        ...
        &.active {
            ${tw`text-indigo-400 bg-indigo-500/10`};
            border-left: 3px solid #6366f1;
            padding-left: 1.35rem;
        }

        & > svg {
            ${tw`w-5 mr-4 text-center transition-colors duration-200 opacity-70`};
        }
...
const LogoutButton = styled.button`
    ${tw`flex items-center w-full px-6 py-3 text-sm font-medium text-neutral-400 transition-all duration-200 cursor-pointer`};
    background: transparent;
    border: none;
    ...
    & > svg {
        ${tw`w-5 mr-4 text-center opacity-70`};
    }
`;
        ...
        <>
            ...
            <div
                ...
                style={{
                    ...
                    left: 0,
                    zIndex: 50,
                    background: '#181b26', // Matches the dark sleek theme from the screenshot
            ...
            >
                ...
                >
                    <Link
                        to={'/'}
                        className={
                            'text-xl font-bold no-underline text-neutral-100 hover:text-white transition-colors duration-150 flex items-center'
                        }
                    >
                        {logo ? (
                            <img src={logo} alt={name} className="w-8 h-8 mr-3 object-contain" />
                        ) : (
                            <svg className="w-8 h-8 mr-3 text-orange-500" viewBox="0 0 512 512" fill="currentColor">
                ...
                <div style={{ display: 'flex', flexDirection: 'column' as const, flex: 1, paddingBottom: '24px', overflowY: 'auto' as const }}>
                    ...
                    <SectionHeader css={tw`mt-4`}>MANAGEMENT</SectionHeader>
                    ...
                    {rootAdmin && (
                        <>
                            <SectionHeader css={tw`mt-4`}>ADMINISTRATOR</SectionHeader>
                            <NavItem>
