    ...
    $('#configTokenBtn').on('click', function (event) {
        $.ajax({
            ...
            swal({
                type: 'success',
                title: 'Token created.',
                text: '<p>To auto-configure and start your node, run the following command:<br /><small><pre>cd /etc/pterodactyl && sudo wings configure --panel-url {{ config(\'app.url\') }} --token ' + data.token + ' --node ' + data.node + '{{ config(\'app.debug\') ? \' --allow-insecure\' : \'\' }} && sudo systemctl enable --now wings</pre></small></p>',
                html: true
            })
        }).fail(function () {
            swal({
